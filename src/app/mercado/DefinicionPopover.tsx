    ...
    <>
      <button
        ...
        className={`shrink-0 w-4 h-4 rounded-full border text-[9px] leading-none flex items-center justify-center transition-colors ${
          open
            ? "border-slate-500 text-cuerpo bg-slate-800"
            : "border-outline text-meta-suave hover:border-slate-500 hover:text-cuerpo"
        }`}
      >
        ?
      </button>
      ...
      {open && pos && createPortal(
        <div
          ...
          className="fixed z-[80] rounded-card border border-outline bg-boton shadow-2xl px-4 py-3"
        >
          <div className="flex items-baseline justify-between gap-2 mb-1.5">
            <h4 className="text-[13px] font-semibold text-titulo">{def.term}</h4>
            <span className="text-[9px] uppercase tracking-widest text-meta-suave whitespace-nowrap">
              {def.categoria}
            </span>
          </div>
          <p className="text-[12px] text-cuerpo leading-relaxed text-pretty">{def.short}</p>
          <Link
            href={hrefGlosario(def.term)}
            className="mt-2.5 inline-block text-[11px] font-medium text-secundario hover:text-titulo transition-colors"
          >
            Ver en glosario →
          </Link>
        </div>,
